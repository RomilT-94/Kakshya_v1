...
public class TrafficMonitorActivity extends Activity {
	...
	public void takeSnapshot(View v) {
		...
		if (latest_rx != null || latest_tx != null) {
		...
		if (previous != null) {
			...
			delta_rx.setText(String.valueOf(latest.device.rx
					- previous.device.rx));
			delta_tx.setText(String.valueOf(latest.device.tx
					- previous.device.tx));
		}
		...
		for (Integer uid : intersection) {
			...
			TrafficRecord previous_rec = (previous == null ? null
					: previous.apps.get(uid));
